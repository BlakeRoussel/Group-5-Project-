...
test1 %>% filter(majorName == "Business Administration") %>%
ggplot() + 
...
mappingFunc <- function(df, name) {
  if (!( name  %in% unique(df$majorName))) stop('You need to pick a hotel within those avaialble in the dataset')
...
mappingFunc(test1, "Business Administration")
```
